...
function hintFunc() {
    let index = randomWord.indexOf(selectedWord)
    if (index === 0) {
...
revealButton.addEventListener("click", () => {
    const index = guessedWord.indexOf(""); // to find the first blank letter
    if (index !== -1) {
        guessedWord[index] = selectedWord[index];
        wordDiv.children[index].textContent = selectedWord[index];

        // Find the letter in the letters list 
